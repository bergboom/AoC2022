...
def movementPart2(storage,input):
    str1 =input.split(' ')
    storage[int(str1[5])-1] = storage[int(str1[3])-1][:int(str1[1])] + storage[int(str1[5])-1]
    storage[int(str1[3])-1] = storage[int(str1[3])-1][int(str1[1]):]
    return storage
...
with open("input05_real.txt") as f:
    lines = f.readlines()
    for line in lines:
        line = line.replace("\n","")
        if(len(line)> 1):
            c1 = str(line[1])
            if(not readColumns):
                storageRow = movementPart2(storageRow,line) #Follow movement
